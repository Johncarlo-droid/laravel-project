  ...
  @if($isCapex)
    ...
    <div class="col-md-4">
      ...
      @if($existingItem?->exists)
        <input class="form-control" value="{{ $existingItem->floor }}" readonly disabled>
        <input type="hidden" name="floor" value="{{ $existingItem->floor }}">
      ...
      @else
        <select name="floor" class="form-select" required>
          ...
          @foreach(($floors ?? []) as $floorOption)
            <option value="{{ $floorOption }}" @selected(old('floor') === $floorOption)>{{ $floorOption }}</option>
          @endforeach
        </select>
      @endif
    </div>
    <div class="col-md-4">
      ...
      <input type="hidden" name="asset_type_name" id="asset_type_name_hidden" value="{{ old('asset_type_name', $item->asset_type_name ?? '') }}">
