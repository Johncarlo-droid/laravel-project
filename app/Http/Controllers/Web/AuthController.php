...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        $validated = $request->validate([
            ...
            // Server-side allow-list: admin/super_admin can NEVER be selected here, regardless of
            // what the form renders. This is the actual security boundary, not the hidden option in the UI.
            'role' => ['required', 'in:requestor,approver,fmo'],
            'approver_type' => ['required_if:role,approver', 'nullable', 'in:dean,executive,adviser,sdao,academic_director'],
        ]);
        ...
        $role = $validated['role'];
        $isAutoApproved = $role === 'requestor'; // requestors: instant, since email verification already happened above
        ...
        $user = User::create([
            ...
            'role' => $role,
            'approver_type' => $role === 'approver' ? $validated['approver_type'] : null,
        ...
        if (!$isAutoApproved) {
            User::whereIn('role', ['admin', 'super_admin'])->get()->each(function (User $admin) use ($user) {
                $admin->notify(new class($user) extends \Illuminate\Notifications\Notification {
                    ...
                    public function toArray(object $notifiable): array {
                        return [
                            ...
                            'message' => $this->newUser->name.' registered as '.ucfirst($this->newUser->role).' and is waiting for approval.',
