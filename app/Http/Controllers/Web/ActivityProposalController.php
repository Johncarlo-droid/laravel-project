...
class ActivityProposalController extends Controller
{
    ...
    public function signDean(ActivityProposal $activityProposal)
    {
        ...
        abort_unless($user->isAdmin() || $user->id === $p->department_approver_id, 403, 'You are not the Dean/Principal assigned to this proposal.');
    ...
    public function signSdao(ActivityProposal $activityProposal)
    {
        ...
        $user = auth()->user();
        abort_unless($p->isAwaitingNoted(), 403, 'This proposal is not currently awaiting "Noted By" signatures (current status: ' . $p->status . ').');
        abort_unless($user->isAdmin() || $user->id === $p->sdao_id, 403, 'You are not the SDAO officer assigned to this proposal (assigned SDAO user id: ' . ($p->sdao_id ?? 'none') . ', your user id: ' . $user->id . ').');
    ...
    public function signFacilities(ActivityProposal $activityProposal)
    {
        ...
        abort_unless($user->isAdmin() || $user->isFmo(), 403, 'You are not a Facilities Management officer.');
    ...
    public function signAcademicDirector(ActivityProposal $activityProposal)
    {
        ...
        $user = auth()->user();
        abort_unless($p->isAwaitingReview(), 403, 'This proposal is not currently awaiting "Reviewed By" signatures (current status: ' . $p->status . ').');
        abort_unless($user->isAdmin() || $user->id === $p->academic_director_id, 403, 'You are not the Academic Director assigned to this proposal.');
    ...
    public function approveExecutive(ActivityProposal $activityProposal)
    {
        ...
        abort_unless($user->isAdmin() || $user->id === $p->executive_director_id, 403, 'You are not the Executive Director assigned to this proposal.');
