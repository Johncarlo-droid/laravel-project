...
class ItemController extends Controller
{
     ...
     * Backed by the asset_types table (admin-extensible: picking "Other, specify" on the
     * create/edit form and typing a new value saves it here for future reuse) instead of a
     * fixed list in code. Falls back to a starter list only if the table is still empty.
     */
    private function assetTypeOptionsArray(): array
    {
        $fromDb = AssetType::with('category')->get()
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($data['asset_type_name']) && !empty($data['category_id'])) {
            AssetType::firstOrCreate([
                'item_category_id' => $data['category_id'],
                'name' => trim($data['asset_type_name']),
            ]);
        }
